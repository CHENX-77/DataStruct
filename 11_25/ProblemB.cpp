...
public:
    ...
    void updatePath(const vector<int>& prePath) {
        ...
        this->path.push_back(indexNo);//TODO: 增加本结点到路径数组this->path中
    }
    ...
    void displayPath(vector<Vertex>& vertexes, int sourceNo) {
        ...
        if (this->distance >= MAX_DIS) {
            cout<<"-1"<<endl;//TODO: 如果源结点到本结点距离无穷大，则（按题目要求）输出-1
            return;
        }
...
public:
    ...
    int getNo(string& label) {
        ...
        for(int j=0;j<vertexes.size();j++)//TODO： 遍历顶点数组vertexes，找到标签属性=label的顶点索引号
            if(vertexes[j].label==label)
                return vertexes[j].indexNo;
            //并返回。
    }
    void readVertexes() {
        //读入每个顶点信息
        ...
        for(; indexNo<n; ++indexNo) {
            ...
            //Vertex(int indexNo, const string& label="", int distance=MAX_DIS)
            //TODO： 创建顶点对象v
            Vertex v(indexNo,label,MAX_DIS);
    ...
    int select() {
        ...
        for(; i<n; ++i) {
            ...
            if (v.distance < minDis) {
                minDis=v.distance;//TODO：记录顶点i的距离和编号到minDis、minNo中
                minNo=v.indexNo;
            }
        }
        if(minNo==-1)
