...
public:
    void readAdjMatrix() {
        ...
        int i=0;
        for(; i<n; ++i) {
            this->isFinished.push_back(false);//TODO：设置this->isFinished数组：每个顶点未曾访问过
    ...
    bool isZeroInDegrees(int vertexNo) {
        ...
        if(i==n)
            return true;//TODO：返回true if 顶点vertexNo没有入度; false [otherwise]
        else
            return false;
    }
    int select() {
        ...
        for (; i < n; ++i) {
            ...
            this->isFinished[i]=true;//TODO: 设置顶点i已经处理过，即isFinished[i]为正确值
            return i;//TODO: 返回i
        }
    ...
    void update(int rootNo) {
        ...
        int j=0;
        for(;j<n; ++j) {
            this->adjMat[rootNo][j]=0;//TODO： 设置adjMat[rootNo][j]为0
        }
    }
    /////////////////////////////////////////////////////
    // 拓扑排序主算法
